...
| `post-start.sh` | On container start: ensure Supabase is running, run Prefect server + worker inside Docker containers, sync `.env.local` via `sync-supabase-env.mjs`. |
