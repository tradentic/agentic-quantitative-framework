...
const LABEL_TO_ENV_KEY = new Map([
  ...
  ['rest url', 'SUPABASE_URL'],
  ['graphql url', 'SUPABASE_GRAPHQL_URL'],
...
function hasRequired(map) {
  return map.has('SUPABASE_URL') && map.has('SUPABASE_ANON_KEY');
}
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (labelKey === 'database url') {
      try {
        ...
        if (!envValues.has('SUPABASE_DB_PASSWORD') && parsedUrl.password) {
          envValues.set('SUPABASE_DB_PASSWORD', parsedUrl.password);
        }
      } catch (error) {
...
function mapStatusKeysToEnv(statusValues) {
  ...
  if (mapped.has('SUPABASE_DB_URL') && !mapped.has('SUPABASE_DB_PASSWORD')) {
    try {
      const parsedUrl = new URL(mapped.get('SUPABASE_DB_URL'));
      if (parsedUrl.password) {
        mapped.set('SUPABASE_DB_PASSWORD', parsedUrl.password);
      }
    } catch (error) {
      console.warn('[sync-supabase-env] Unable to parse database URL for password extraction:', error.message);
    }
...
async function main() {
  ...
  if (!hasRequired(updates)) {
    const missing = [];
    if (!updates.has('SUPABASE_URL')) missing.push('SUPABASE_URL');
    if (!updates.has('SUPABASE_ANON_KEY')) missing.push('SUPABASE_ANON_KEY');
